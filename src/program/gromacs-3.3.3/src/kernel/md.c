...
size_t getNumberOfLines()
{
  ...
  printf("Read %zu time slices from profile.txt\n", n);

  return n;
 }

int readProfile(real *t_profile, real *inten_profile)
{
  FILE *fp = fopen("profile.txt", "r");
  if (fp == NULL)
  {
	     ...
	     unsigned long Flags)
{
  ...
  size_t n_profile = getNumberOfLines();

  t_profile = (real *) malloc( n_profile * sizeof(real) );
  inten_profile = (real *) malloc( n_profile * sizeof(real) );

  readProfile(t_profile, inten_profile);
  ...
  while ((!bRerunMD && (step_rel <= inputrec->nsteps)) ||  
    ...
    if (bIonize){
      ionize(log,mdatoms,top->atoms.atomname,t,inputrec,state->x,state->v,
	     START(nsb),START(nsb)+HOMENR(nsb),state->box,cr,t_profile,inten_profile,n_profile);
	}
